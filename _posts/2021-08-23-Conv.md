...
1D convolution operator can be represented as a matrix multiplication of [Toeplitz matrix] [toeplitz_matrix] representation of kernel weights $$ T_{w} $$, and transposed input X:
...
$$
\frac {\partial E} {\partial \hat Y} = J = \left( \partial \hat y_{0}, \partial \hat y_{1},  \ldots \partial \hat y_{n-1} \right)
$$


And using Toeplitz matrix multiplication for Y, convolution, we see that derivagtive of convolution is tansposed matrix $$ T_{x} $$:
...
$$
\frac {\partial \hat Y} {\partial W} = T_{x}^\intercal
$$


Kernel weight gradient $$ \frac {\partial E} {\partial W} $$ becomes convolution of convolution input $$ X $$ and filter $$ \frac {\partial E} {\partial \hat Y} $$ :

$$
\frac {\partial E} {\partial W} = J T_{x}^\intercal = \left( \partial \hat y_{0}, \partial \hat y_{1},  \ldots, \partial \hat y_{n-1} \right)  \left( \begin{array}{ccc}
x_{0}    &      0    &  \ldots &        0   \\
x_{1}    &  x_{0}    &  \ldots &        0   \\
...
One can notice that $$ \frac {\partial E } {\partial W} $$ is convolution of error derivative \frac {\partial E} {\partial \hat Y} and input X
...
$$
\frac {\partial \hat Y} {\partial X} = \frac {\partial (X T_{w})} {\partial X} = T_{w}^\intercal
$$

$$
\frac {\partial E (\hat Y) } {\partial X} = J T_{w}^\intercal = \left( \partial \hat y_{0}  \partial \hat y_{1}   \ldots  \partial \hat y_{n} \right)  \left( \begin{array}{ccc}
 w_{0}  &  w_{1}  &  \ldots &    w_{m}  &        0  &       0  \\
     0  &  w_{0}  &   w_{1} &   \ldots  &  w_{m-1}  &       0  \\
\ldots  & \ldots  &  \ldots &   \vdots  &   \ldots  &  \ldots  \\
     0  &      0  &   w_{0} &   \ldots  &  w_{m-2}  & w_{m-1}  \\
\end{array} \right)
$$
