...
simpler cross-corellation instead of "proper" convolution:
...
And using multiplication $$ T_{x} W $$ for convolution, we can see that partial derivative
of convolution $$ \frac {\partial \hat Y} {\partial W} $$ is matrix $$ T_{x} $$:

$$
\frac {\partial \hat Y} {\partial W} = \frac {\partial (W * X)} {\partial W}  = \frac {\partial (W T_{x})} {\partial W} = T_{x}
$$

Kernel weight gradient $$ \frac {\partial E} {\partial W} $$ becomes convolution of input $$ X $$ and gradient $$ \frac {\partial E} {\partial \hat Y} $$ :
...
#### Include headers, add print helper functions for printing 2D arrays
...
#### Add weigts and bias initializers: we'll use incremental counters to initailze weights similar to ear;ier Python code
...
#### Using mean squared error for loss functions
...
#### Error function expects 1 D input. We'll introduce Flatten layer to
reshape 2D output of convolution into single dimention vector.

Backpropagation for this layer is inverse reshape from 1D to 2D shape
...
#### 2D convolution layer class template is created with
...
#### Main functin creates a neural net consisting of one convolution, one flattent, and
mean squared error loss.

After executing forward path, it prints output and loss.
Then it backpropagates the loss and prints updatd convolution layer weights and bias values.
One can verify that updated weights amd biases match those in the earlier Python implementation.
...
[Matrix coockbook] [matrix_coockbook]
