...
The kernel in convoution is flipped. The reason for this is to maintain commutative property:
...
Kernel weight gradient $$ \frac {\partial E} {\partial W} $$ becomes convolution of Conv layer input $$ X $$ and gradient $$ \frac {\partial E} {\partial \hat Y} $$ :

$$
\frac {\partial E} {\partial W} = J T_{x} = \left( \begin{array}{ccc}
\partial \hat y_{0} & \partial \hat y_{1} & \ldots & \partial \hat y_{n-1} \\
\end{array} \right)   \left( \begin{array}{ccc}
  x_{0}   &      x_{1}  &  \ldots &  x_{k-1}  \\
  x_{1}   &      x_{2}  &  \ldots &    x_{k}  \\
...
### Output gradient $$ \frac {\partial E (\hat Y) } {\partial X} $$ is convolution of error gradient J and kernel weighs rotated 180 degrees
