...
Continuos convolution of two functions f and g is a function:
...
Using Jacobian for derivative of error E with respect to $$ \hat Y $$ :

$$
J = \frac {\partial E} {\partial \hat Y} =  \left( \partial \hat y_{0}, \partial \hat y_{1},  \ldots \partial \hat y_{n-1} \right)
$$


And using Toeplitz mmatrix multiplication for Y:

$$
\frac {\partial \hat Y} {\partial W} = \frac {\partial (W * X)} {\partial W}  = \frac {\partial (W T_{x})} {\partial W}
$$

We get:

$$
\frac {\partial \hat Y} {\partial W} = J T_{x}^\intercal
$$
...
$$
\frac {\partial E (\hat Y) } {\partial W} = J T_{x}^\intercal = \left( \partial \hat y_{0}, \partial \hat y_{1},  \ldots, \partial \hat y_{n-1} \right)  \left( \begin{array}{ccc}
x_{0}    &      0    &  \ldots &        0   \\
x_{1}    &  x_{0}    &  \ldots &        0   \\
