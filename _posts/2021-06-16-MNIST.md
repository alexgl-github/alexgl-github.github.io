...
I'll focus on C++ implementation only. Python equivalent won't be added because there's already many existing MNIST classification examples
...
struct mnist
{
  ...
  /*
   * Read next image label and return it as one-hot encoded array
   */
...
![mnist_image]({{ site.url }}/images/f1.png)
...
### C++ implementation for F1 score calculation
...
is used in backward() function, so thet we don't need  transpose Dense layer weights in each backward() function call.

Include heared files
...
In this example dense layer weiights are initialized with random uniform initializer,
...
struct Dense
{
  ...
  /*
   * Update Dense layer weigts/biases using accumulated dw and db
   */
...
epoch 4/5 time/epoch=96.00000 sec; time left=0.0000 hr; avg loss=0.13408; f1=0.96033
