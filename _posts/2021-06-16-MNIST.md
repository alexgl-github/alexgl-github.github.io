...
Let's train this DNN to recognize handwritten digit classification problem.
...
I''l focus on C++ implementation only.  Python implementation won't be added because this is a very popular toy problem available in mutiple examples and in all major frameworks.
...
Dataset images and labels binary files have the format as follows.
...
After header containing magic number and total number of labels in the dataset, there are label records. Each record contains one byte label.
Each label is between 0 and 9 indicating handwritten digit value.
...
After header containing magic number and total number of images in the dataset, there are images records.
Each image record contains pixel values for one image of size 28 rows by 28 columns, in row scan order format.
Pixels are one pixel per byte with values between 0 and 255


Using this formt information, we can implement C++ code mnist.h for reading MNIST images and labels.

Header files used in the implementation
...
Template C++ class definiton for reading MNIST dataset
...
To validate training results I'll use multi-class F1 score
...
Main C++ module implementing DNN layers, training and validation loops, and reporting loss and F accuracy oin train and validation datasets
...
Previous example can be found at ["Dense layer with backpropagation, part 6. Cross-entropy Error"] [part_6]

C++ implementation is at [dense7.cpp] [cpp_source_code]
