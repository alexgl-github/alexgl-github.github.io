...
E(\hat Y, Y) = - \sum_{i=9}^{M-1} y_{i} * log(\hat y_{i})
...
\frac {\partial E(\hat Y, Y)} {\partial y_{i}} = \frac {\partial ( y_{i} * log(\hat y_{i}))} {\partial \hat y_{i}} \\
\frac {\partial E(\hat Y, Y)} {\partial y_{i}} = \frac {y_{i}} {\hat y_{i}}
...
struct CCE
{
  ...
  /*
   * Forward pass computes CCE loss for inputs yhat (predicted) and y (label)
   */
...
int main(void)
{
  ...
  /*
   * Print DNN output y and expected output ytrue
   */
  printf("output y=");
