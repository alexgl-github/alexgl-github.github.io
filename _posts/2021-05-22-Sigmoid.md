...
### First, let's modify python code from previous example to use Sigmoid activation in dense layers.
I'll use it to validate C++ code in the consecutive section.
...
Create 2 layer sequential model. Te only difference from the previous post example is
using sigmoid activation in the dense layer.
